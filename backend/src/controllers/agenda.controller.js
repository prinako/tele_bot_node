...
async function create(req, res, next) {
    try {
        ...
        if (!agenda) {
            return res.status(400).json({ error: 'Agenda payment could not be created' });
        }
...
async function listByUser(req, res, next) {
    try {
        res.json(await agendaService.getAgendaPaymentsByUser(req.params.telegramId));
    } catch (error) {
        next(error);
    }
}
...
async function patch(req, res, next) {
    try {
        ...
        const agenda = await agendaService.updateAgenda(req.params.id, req.body, telegramId);
        if (!agenda) {
            return res.status(404).json({ error: 'Agenda payment not found' });
        }
