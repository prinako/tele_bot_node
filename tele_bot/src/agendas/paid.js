...
class paid {
  ...
  async _getAllAgendas(msg) {
    ...
    const buttons = await allAgendaAsKeyboard(msg.from.id, "paid");
  ...
  async addToDatabase(callbackQuery, data) {
    ...
    if (updateAgenda) {
      const alert =
        `A conta deª *${updateAgenda.title}* no valor de *${updateAgenda.amount}* com vencimento para *${updateAgenda.date}* foi marcada como paga. ✅`;
