...
import { insetPix, updatePix } from "../api/backendClient.js";
...
class AddPixToDB {
  ...
  set isStagePixChave(v) {
    this._isStagePixChave = v;
  }

  get isStagePixChave() {
    return this._isStagePixChave;
  }
    ...
    await insetPix(data, (result) => {
      ...
      if (result.error) {
        ...
        if (errorCode === "23505" || errorCode === 11000) {
          ...
          this.bot.sendMessage(
            userId,
            `Seu Pix chave ${this.pix} já existente no banco de dados \nPor favor, cadastre um novo pix.`,
            {
              message_thread_id: messageThreadId,
            },
          );
          ...
          this.bot.sendMessage(
            userId,
            "Erro ao adicionar pix!, Por favor, tente novamente",
            {
              message_thread_id: messageThreadId,
            },
          );
